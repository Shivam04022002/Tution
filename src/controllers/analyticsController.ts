...
export const getOverviewAnalytics = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [
      ...
      TutorApplication.countDocuments({ status: 'accepted' }),
      ...
      ScheduledClass.countDocuments({ status: 'active' }),
      ...
      ParentRequirement.countDocuments({ status: 'active', createdAt: { $gte: monthStart } }),
