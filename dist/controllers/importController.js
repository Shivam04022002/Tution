...
exports.getImportHistory = exports.importTeachersExcel = exports.importParentsExcel = void 0;
const XLSX = __importStar(require("xlsx"));
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const User_1 = require("../models/User");
const TeacherProfile_1 = require("../models/TeacherProfile");
const ImportHistory_1 = require("../models/ImportHistory");
...
const importParentsExcel = async (req, res) => {
    const adminId = req.user?._id;
    if (!req.file) {
        return res.status(400).json({ success: false, message: 'No file uploaded' });
    }
    const fileName = req.file.originalname;
    const history = await ImportHistory_1.ImportHistory.create({
        uploadedBy: adminId,
        fileName,
        importType: 'parents',
        totalRows: 0,
    ...
    try {
        ...
        const [existingEmails, existingMobiles] = await Promise.all([
            User_1.User.find({ email: { $in: candidateEmails } }).select('email').lean(),
            User_1.User.find({ phoneNumber: { $in: candidateMobiles } }).select('phoneNumber').lean(),
        ]);
        const emailSet = new Set(existingEmails.map((u) => u.email.toLowerCase()));
        const mobileSet = new Set(existingMobiles.map((u) => u.phoneNumber));
        for (let i = 0; i < rows.length; i++) {
            ...
            if (!name) {
                rowErrors.push({ rowNumber, rowData: row, errorMessage: 'Name is required' });
                continue;
            }
            if (!email || !/^\S+@\S+\.\S+$/.test(email)) {
                rowErrors.push({ rowNumber, rowData: row, errorMessage: 'Invalid or missing email' });
                continue;
            }
            if (!mobile || mobile.length < 10) {
                rowErrors.push({ rowNumber, rowData: row, errorMessage: 'Invalid or missing mobile number' });
                continue;
            }
            ...
            const tempPassword = await bcryptjs_1.default.hash(mobile.slice(-6), 10);
            toInsert.push({
                email,
                phoneNumber: mobile,
        ...
        if (toInsert.length > 0) {
            const result = await User_1.User.insertMany(toInsert, {
                ordered: false,
                rawResult: true,
            });
            successfulRows = Array.isArray(result) ? result.length : result.insertedCount ?? toInsert.length;
        }
        const failedRows = rowErrors.length;
        const status = successfulRows === totalRows - duplicates
            ? 'completed'
            : successfulRows === 0
                ? 'failed'
                : 'partial';
        await ImportHistory_1.ImportHistory.findByIdAndUpdate(history._id, {
            totalRows,
            successfulRows,
            failedRows,
            duplicates,
        ...
        await ImportHistory_1.ImportHistory.findByIdAndUpdate(history._id, { status: 'failed' });
...
exports.importParentsExcel = importParentsExcel;
const importTeachersExcel = async (req, res) => {
    const adminId = req.user?._id;
    if (!req.file) {
        return res.status(400).json({ success: false, message: 'No file uploaded' });
    }
    ...
    try {
        ...
        if (toInsertUsers.length > 0) {
            await User_1.User.insertMany(toInsertUsers, { ordered: false });
            await TeacherProfile_1.TeacherProfile.insertMany(toInsertProfiles, { ordered: false });
        ...
        try {
            ...
            await (0, notificationService_1.notifyAdminImportCompleted)(adminIds, 'Teachers', successfulRows, history._id);
        }
        catch (_) { }
        return res.status(200).json({
            success: true,
...
exports.importTeachersExcel = importTeachersExcel;
const getImportHistory = async (req, res) => {
    try {
        const { importType, status, page = '1', limit = '20', } = req.query;
        const filter = {};
        if (importType)
        ...
        const [records, total] = await Promise.all([
            ImportHistory_1.ImportHistory.find(filter)
                .select('-rowErrors')
                ...
                .limit(limitNum)
                .lean(),
            ImportHistory_1.ImportHistory.countDocuments(filter),
        ]);
        return res.status(200).json({
            success: true,
            data: records,
