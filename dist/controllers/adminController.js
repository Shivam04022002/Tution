...
exports.unblockTeacher = exports.deleteParent = exports.updateParent = exports.getParentDetails = exports.getAllParents = exports.blockTeacher = exports.rejectTeacher = exports.approveTeacher = exports.getTeacherDetails = exports.getAllTeachers = exports.getAllUsers = exports.getPlatformStats = void 0;
...
async function writeAuditLog(adminId, action, entityType, entityId, oldValue, newValue, req) {
    try {
...
const getPlatformStats = async (req, res) => {
    try {
        const [totalParents, totalTeachers, pendingTeachers, activeRequirements, totalApplications, totalDemoClasses,] = await Promise.all([
            ...
            TutorApplication_1.TutorApplication.countDocuments({ isActive: true }),
            DemoClass_1.DemoClass.countDocuments({ isActive: true }),
        ]);
...
const getAllTeachers = async (req, res) => {
    try {
        const { verificationStatus, city, search, page = '1', limit = '20', } = req.query;
        const filter = {};
        if (verificationStatus)
            filter.verificationStatus = verificationStatus;
        if (city)
            filter['locationAvailability.city'] = { $regex: city, $options: 'i' };
        if (search) {
            filter.$or = [
        ...
        const [teachers, total] = await Promise.all([
            TeacherProfile_1.TeacherProfile.find(filter)
                .select('basicDetails.fullName basicDetails.email basicDetails.mobileNumber basicDetails.profilePhoto ' +
                ...
                .limit(limitNum)
                .lean(),
            TeacherProfile_1.TeacherProfile.countDocuments(filter),
        ]);
        return res.status(200).json({
            success: true,
            data: teachers,
...
const approveTeacher = async (req, res) => {
    try {
        ...
        await teacher.save();
        await User_1.User.findByIdAndUpdate(teacher.userId, { isVerified: true });
        await writeAuditLog(adminId, 'APPROVE_TEACHER', 'TeacherProfile', id, oldValue, { verificationStatus: 'verified', isVerified: true }, req);
        return res.status(200).json({
            success: true,
...
const rejectTeacher = async (req, res) => {
    try {
        ...
        await teacher.save();
        await User_1.User.findByIdAndUpdate(teacher.userId, { isVerified: false });
        await writeAuditLog(adminId, 'REJECT_TEACHER', 'TeacherProfile', id, oldValue, { verificationStatus: 'rejected', rejectionReason: reason.trim() }, req);
        return res.status(200).json({
            success: true,
...
const blockTeacher = async (req, res) => {
    try {
        ...
        await teacher.save();
        await User_1.User.findByIdAndUpdate(teacher.userId, { isActive: false });
        await writeAuditLog(adminId, 'BLOCK_TEACHER', 'TeacherProfile', id, oldValue, { isBlocked: true, blockReason: reason.trim(), isActive: false }, req);
        return res.status(200).json({
            success: true,
...
exports.blockTeacher = blockTeacher;
const getAllParents = async (req, res) => {
    try {
        ...
        const parentIds = users.map((u) => u._id);
        const reqCounts = await ParentRequirement_1.ParentRequirement.aggregate([
...
const getParentDetails = async (req, res) => {
    try {
        const { id } = req.params;
        const user = await User_1.User.findOne({ _id: id, role: 'parent' })
            .select('-password -firebaseUid -__v')
            .lean();
        if (!user) {
            return res.status(404).json({ success: false, message: 'Parent not found' });
        }
        const requirements = await ParentRequirement_1.ParentRequirement.find({ parentId: id })
...
const updateParent = async (req, res) => {
    try {
        ...
        const updated = await User_1.User.findByIdAndUpdate(id, { $set: updates }, { new: true, runValidators: true }).select('-password -firebaseUid -__v');
        await writeAuditLog(adminId, 'UPDATE_USER', 'User', id, oldValue, updates, req);
        return res.status(200).json({
            success: true,
...
const deleteParent = async (req, res) => {
    try {
        ...
        await User_1.User.findByIdAndUpdate(id, { isActive: false });
        await ParentRequirement_1.ParentRequirement.updateMany({ parentId: id }, { isActive: false, status: 'closed' });
        await writeAuditLog(adminId, 'DELETE_USER', 'User', id, oldValue, { isActive: false }, req);
        return res.status(200).json({
            success: true,
...
const unblockTeacher = async (req, res) => {
    try {
        ...
        await teacher.save();
        await User_1.User.findByIdAndUpdate(teacher.userId, { isActive: true });
        await writeAuditLog(adminId, 'UNBLOCK_TEACHER', 'TeacherProfile', id, oldValue, { isBlocked: false, isActive: true }, req);
        return res.status(200).json({
            success: true,
...
exports.unblockTeacher = unblockTeacher;
